...
- **Save.** `wiki_save` writes a page into the shared wiki and `wiki_archive` files an answer
  ...
  covers the lot. Large documents are split deterministically into model-safe source parts.
  Conversion itself stays outside the runtime, which has to fit on a Pi.
- **Ingest.** `brain_capture` files a source into the current chat's `raw/` and queues it —
  ...
  source it came from. Transient malformed output retries up to three times; an owner can
  explicitly re-queue a terminal failure with `wiki_retry_source`. Sources that add nothing
...
- **Query.** `wiki_search` and `wiki_answer` search the index and full text and answer with
  validated citations to pages the answer actually used. `wiki_archive` files a good answer back as a snapshot page, so explorations
